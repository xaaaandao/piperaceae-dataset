...
export default function Datasets({dataset}) {
    ...
    function TableSpecies() {
        const url = `http://localhost:3000/piperaceae-dataset/data/${dataset.toString().toLowerCase()}_dataset/5/info_levels.json`;
    ...
    function TableSamples() {
        const url = `http://localhost:3000/piperaceae-dataset/data/${dataset.toString().toLowerCase()}_dataset/5/info_samples.json`;
        console.log(url);
        const { data, error, loading } = useFetch(url, options);
        if (loading) {
            return <p>Loading...</p>;
    ...
    return (
        <FormControl>
            <Text>This tab provides data of Samples {dataset}.</Text>
            <Stack my={5}>
                <Stack direction="row">
                    ...
                    {dataset==="Regions" && layoutSelect(handleChangeRegion, "Regions", regions, "Choose the regions", region)}
                </Stack>
                <Button onClick={handleClick}>Get the link!</Button>
                {select && showLink()}
                <Accordion allowToggle>
                    <AccordionItem isDisabled={disableTable}>
                        <h2>
                        ...
                        <AccordionPanel pb={4}>
                            <Text>Any the samples used are available in this file.</Text>
                            {TableSamples()}
                        </AccordionPanel>
                    </AccordionItem>
                    ...
                    <AccordionItem isDisabled={disableTable}>
                        ...
                        <AccordionPanel pb={4}>
                            <Text>Any the species chosen are available in this file.</Text>
                            {TableSpecies()}
                        </AccordionPanel>
                    </AccordionItem>
                </Accordion>
